...
2. Clarified the Hive table definitions
...
Which format your SnowPlow event files are in will depend on how your EmrEtlRunner is configured. If your `config.yml` has:
...
Whereas if your `config.yml` has:

    :storage_format: non-hive

then your SnowPlow events will be stored in the format shown in [`hive-format-table-def.q`] [hive-format-format].
