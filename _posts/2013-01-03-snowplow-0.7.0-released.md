...
Interested? Read on below the fold for installation instructions and some additional information on this release.
...
You will find full instructions on setting up the new Clojure-based collector in our wiki, [Setting up the Clojure collector] [clj-collector-setup].

### ETL

If you are using EmrEtlRunner, you need to update to the latest version, which is 0.0.7 (available by checking out the master branch of the [repository] [snowplow-repo]).
...
If you are using StorageLoader, you need to update to the latest version, which is 0.0.3 (available by checking out the master branch of the [repository] [snowplow-repo]).
...
Running this script will create a new table, `events_005` (version 0.0.5 of the table definition) in your `snowplow` database, copying across all your data from your existing `events` table, which will not be modified in any way.
...
2. Duplications - the two collectors may be occassionally duplicating different rows (see [issue xx] [issue-xx])
3. Browser differences - it may be that the user navigates away from the page before one or other collector can register the event

We plan to test this further with larger datasets; we also plan on exploring the duplicate rows to check there are no issues there.

## Other features in this release

There are a few minor changes in this release not related to the Clojure-based collector.

Both EmrEtlRunner and StorageLoader now print "Completed successfully" to `stdout` on completion. This should help make it clearer (e.g. in logs) that these Ruby programs have completed successfully.
