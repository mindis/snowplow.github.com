...
We're very pleased to announce the release of Snwowplow 0.8.11. This releases includes two different sets of updates:

1. **Critical update**: support for Amazon's new Cloudfront log file format. (Rolled out by Amazon yesterday.)
...
2. [IP anonymization] (/blog/2013/10/22/snowplow-0.8.11-released-supports-all-cloudfront-file-formats-and-other-improvements/#ip)
...
|:-------------------------------|:---------------------------------------------------------------------|
| Cloudfront pre-August 17 format | The original CloudFront log file format, around which Snowplow was orignially developed. |
| August 17 unnanounced upgrade  | Surprise change in CloudFront log file format - see [the Google Group] [original-change] for details. |
| September 14 fix               | A new format, different to the previous two. For details, see [this forum thread] [forum-thread]. |
| October 21 update              | Amazon updated the log file format with three new fields, as per [this post] [latest-update], including three new fields. |

<!--more-->

The latest version of Snowplow supports *all* the different versions of the file format listed above, including that one rolled out yesterday. It is important to note that the October 21st CloudFront file format is **not** supported by previous Snowplow versions: as a result, we'd expect existing Snowplow users using the CloudFront collector to see a significant number of lines in their bad rows bucket in S3 with the following format:
...
<a name="ip"><h2>2. IP Anonymization</h2></a>
...
Upgrading is a four step process:

4.1 [Update the EmrEtlRunner] (#hadoop-etl)
4.2 [Update StorageLoader config file] (#storageloader) 
4.3 [Reprocess any "bad rows"] (#reprocess) collected from before the upgrade was completed  

Let's take these in term:

<a name="hadoop-etl"><h3>4.1 Update the EmrEtlRunner</h3></a>
...
To enable IP enrichment, you need to set `anon_ip.enabled` to true, and specify the level of anonymization with `anon_ip.anon_octets` field. If, for example, my IP address is '37.157.33.178', then setting it to different values between 1 and 4 would anonymize my IP address as follows:

| `enrichments.anon_ip.anon_octets` value | IP address displayed in Snowplow |
|-----------------------------------------|----------------------------------|
| 0                                       | 37.157.33.178                    |
| 1                                       | 37.157.33.x                      |
| 2                                       | 37.157.x.x                       |
...
<a name="storageloader"><h3>4.2 Update the StorageLoader</h3></a>
...
The StorageLoader `config.yml` file includes a new `:comprows:` option for Redshift users. This determines the number of rows that Amazon analyzes in order to determine the best encoding format to use for the different fields in your Redshift tables. Note that this is **only** used if the `--include compupdate` option is specified when running the StorageLoader. For more information on Amazon's `comprows` functionality, see the [Redshift documentation] [comprows-redshift].
...
As described above, if you've been using the CloudFront collector, you will have a number of rows of data in your "bad bucket" on S3 generated after the new CloudFront log file format was rolled out on October 21st, because these data rows were not supported by the old version of the Snowplow.
