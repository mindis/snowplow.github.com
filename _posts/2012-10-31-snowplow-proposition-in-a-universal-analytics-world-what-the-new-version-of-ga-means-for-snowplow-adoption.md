...
Previously, Google Analytics identified unique users using their own `cookie_id`s. Google's `cookie_id`s are an excellent starting point for identifying users, as many have Google accounts. (Thanks to Google's myriad mass-market services, including Gmail, Youtube, Google Play / Android etc.) Because consumers using these services on multiple devices identify themselves to Google when they login, Google can marry their `cookie_id`s for these users on all the different devices they use. As a result, Google is one of the few companies in the world to be able to reliably identify users across multiple platforms and devices. (Other notables in the same position include Facebook, Amazon, Paypal and Microsoft.)

For businesses using GA, being able to augment Google's user identification with their own internal `customer_id`s is a significant step forwards for two reasons:

1. Many GA users (especially those with applications or properties where users login, or those with a loyalty card scheme) can identify their own users reliably on specific platforms, devices and physical stores. By adding this additional user identification intelligence into GA, GA will be more accurate at identifying the same user in different places reliably, moving us further from a world in which we rely on persistent cookies dropped on browsers with unique identifiers, to one where users are more robustly identified via logins, payments and loyalty schemes. (Which still use cookies, but as part of a broader set of user identification business processes that actively involve the user in the identification process.)
...
* Does it make sense to provide detailed event and customer-level data to a company that wont share the data back with its users in the same atomic level of detail. (Google insists on rolling the data up to aggregates, that are less flexible to work with from an analytics perspective.)
