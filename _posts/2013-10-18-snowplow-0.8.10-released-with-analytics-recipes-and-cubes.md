...
One of the things we've learnt from many new Snowplow users, is that they want to get up and running analyzing Snowplow data as fast as possible: oftenby slapping a familiar business intelligence tool directly on top of Snowplow data, to start exploring and visualizing that data.

For those users, one of the frustrations with Snowplow, is that analysis typically starts with a SQL query on the Snowplow data, to transform it into a format suitable for analysing in a BI / OLAP tool. Whilst we believe it is a strength that Snowplow gives you the flexility to design and structure a wide range of different analyses, we recognise that for new users in particular, it would be nicer if they could dive straight into the data in our BI tool of choice.)

This release aims to bridge that gap. We've provided a range of recipes and cubes as views into the atomic Snowplow data. All of these are suitable for being loaded into BI tools like Excel, ChartIO and Tableau, directly.
...
1. The `events` table is now located in the `atomic` schema. (This is new for Redshift users.)
...
Finally, you will want to add the new schemas to your `search_path`. This is necessary if the views in them will show up in tools like Tableau and SQL Workbench. An explanation of how to update the search path is given [here, in the setup guide] [search-path-setup].
...
Afterwards, you can create the new schemas and views, by runnign the following scripts:
