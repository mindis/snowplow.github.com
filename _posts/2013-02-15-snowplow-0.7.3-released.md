...
* Silently checks for user activity (mouse movements, scrolling, key presses etc) on a page for a certain time period (e.g. 10 seconds)
...
**Simply put: these new offset fields are designed to provide a very clear view of how your users scroll around your webpages over time.**
...
1. Our `logImpression()` method was not working (was using the wrong argument names) - this has now been fixed.
2. The activity tracking (page ping) behaviour was too fragile: if a single monitoring period elapsed with no activity, then monitoring would be cancelled. This could easily lead to on-page activity not being recorded. This has now been fixed
