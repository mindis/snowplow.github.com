...
2. "Visit" is very website-specific compared to "session" which works for mobile, desktop, TV analytics etc
3. This session index is generated within the JavaScript tracker, by setting a first party cookie. By calling it `domain_sessionidx` we make it clearer that this depends on a first party cookie and tracks a visit at the domain level, not across a network of domains
