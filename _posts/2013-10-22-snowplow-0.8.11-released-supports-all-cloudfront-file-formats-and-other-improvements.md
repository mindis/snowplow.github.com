...
* Add `--debug` to your EmrEtlRunner invocation to make EMR job debugging available for this ETL job
* Add `--include vacuum` to your StorageLoader invocation if you want to include a `VACUUM` step after your table load
* Add `--skip analyze` to your StorageLoader invocation if you **don't** need to run a table `ANALYZE` step after your table load
* Add `--include compupdate` to your StorageLoader invocation if you want to (re-)generate the compression encodings on your table's fields. This setting uses the new `:comprows:` parameter in the `config.yml` file - see [s]ection 4.2](#storage-loader) below for details
